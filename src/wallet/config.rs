...
pub struct Config {
    ...
    /// DYNAMIC_EMBEDDED_POC: shared limits for externally signed sends; retain for all providers.
    #[serde(default)]
    pub mpc_send: MpcSendPolicy,
}

/// DYNAMIC_EMBEDDED_POC: shared policy saved with each send so configuration changes cannot
/// change the fee or amount policy of an already prepared transaction.
...
impl Default for MpcSendPolicy {
    fn default() -> Self {
        // Preserve the existing test deployment and legacy operation journals.
        Self {
            max_amount: 25,
            fee_rate_sat_vb: 2,
            max_fee_sat: 2000,
